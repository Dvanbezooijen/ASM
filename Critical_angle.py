...
for i,test in enumerate(drained_tests):
    G0_modulus,G2_modulus,G50_modulus,q_f,p_f,V0 = Shearing_phase_CID(
...
for i,test in enumerate(undrained_tests):
    G0_modulus,G2_modulus,G50_modulus,q_f,p_f,V0 = Shearing_phase_CIU(
...
plt.title(f'Stress envelope of triaxial tests')
plt.grid(True)
plt.show()
